...
async def comp_create_ticket():
    ...
    return redirect(url_for("comp_tickets"))


@app.get("/ticket/<int:index>/details")
async def comp_ticket_details(index: int):
    with Config.SESSION.begin() as session:
...
async def delete_ticket():
    ...
    return redirect(url_for(comp_tickets.__name__))